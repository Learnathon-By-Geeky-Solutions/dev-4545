...
const TableActions: React.FC<Props> = ({ onStatusChange, onDelete }) => {
  ...
  const getActions = (userId: number): MenuProps["items"] => [
    {
      key: `set-status-${userId}`,
      label: "Set Status",
      children: [
        {
          key: `set-status-pending-${userId}`,
          label: "Pending",
          onClick: () => onStatusChange(userId, "pending"),
        },
        {
          key: `set-status-in-progress-${userId}`,
          label: "In Progress",
          onClick: () => onStatusChange(userId, "in-progress"),
        },
        {
          key: `set-status-done-${userId}`,
          label: "Done",
          onClick: () => onStatusChange(userId, "done"),
        },
      ],
    },
    {
